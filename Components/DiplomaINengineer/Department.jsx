...
export const DepartmentBanner = ({ department, ID }) => {
  ...
  const [filteringValue, setfilterValue] = useState("");
  const [ResultBooks, setResultBooks] = useState("");

  useEffect(() => {
    if (ID === "CT") {
      setSearchingValue("সিভিল টেকনোলজি");
    }
    if (ID === "CST") {
      setSearchingValue("কম্পিউটার সাইন্স এন্ড টেকনোলজি");
    }
    if (ID === "ET") {
      setSearchingValue("ইলেকট্রিক্যাল টেকনোলজি");
    }
    if (ID === "MT") {
      setSearchingValue("মেকানিক্যাল টেকনোলজি");
    }
    if (ID === "EMT") {
      setSearchingValue("ইলেকট্রোমেডিক্যাল টেকনোলজি");
    }
    if (ID === "ETT") {
      setSearchingValue("ইলেকট্রনিক্স টেকনোলজি");
    }
    if (ID === "PT") {
      setSearchingValue("পাওয়ার টেকনোলজি");
    }
    ...
    fetchData();
  }, [searchingValue, filteringValue, ID]);

  const handleClick = (semester) => {
    setActiveBorder(semester);
    setfilterValue(semester);
  };
